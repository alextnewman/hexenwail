...
| Document | Covers |
| --- | --- |
| [`docs/web/ARCHITECTURE.md`](docs/web/ARCHITECTURE.md) | What the web port is, its non-goals, layering, the web renderer macro contract, ownership boundaries |
...
| [`docs/web/WEBGLIDE_NITRO.md`](docs/web/WEBGLIDE_NITRO.md) | WebGlideNitro, the primary native WebGPU renderer (`-DWEB_RENDERER=webgpu`), measured on the target iPad against its own captures |
...
* **WebGlideNitro is the default**. The classic software renderer remains
  available as the correctness reference and uses WebGPU for presentation.
  Nitro performance is measured directly on the target iPad against Nitro's
  own captures.
