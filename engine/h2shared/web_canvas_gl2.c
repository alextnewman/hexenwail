...
void WebCanvas_Init (void)
{
	...
	/* The presented image is fully opaque and never read back, so every
	 * ancillary buffer and every implicit copy is pure overhead. */
	...
	if (webgl_context <= 0)
		Sys_Error("Unable to create a WebGL2 context for the game canvas");
